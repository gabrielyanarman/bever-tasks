...
async function initializePriceList(formContext) {
    ...
    try {
        ...
        const filteredResult = fetchResult.entities.filter((item) => item["pli.new_fk_price_list"] === correctId(priceListId))
        
        const currencyLookup = formContext.getAttribute("transactioncurrencyid").getValue();
        ...
        const products = fetchResult.entities.reduce((acc, product) => {
          if (!acc.some((item) => item["new_productid"] === product["new_productid"])) {
            acc.push(product);
          }
          return acc;
        }, []);
        ...
        filteredResult.forEach(async (product) => {
            await Xrm.WebApi.deleteRecord("new_price_list_item", product["pli.new_price_list_itemid"]);
        })

        products.forEach(async (product) => {
            const data = {
              ["new_name"]: product["new_name"],
              ["new_fk_price_List@odata.bind"]: `/new_price_lists(${correctId(priceListId)})`,
              ["new_fk_product@odata.bind"]: `/new_products(${product["new_productid"]})`,
              ["transactioncurrencyid@odata.bind"]: `/transactioncurrencies(${correctId(currencyId)})`,
